...
public class Espetaculo {
	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim, LocalTime horario, Periodicidade periodicidade) {
		...
		int totalDias = Days.daysBetween(inicio, fim).getDays() + 1;

		if (periodicidade == Periodicidade.DIARIA) {

			for (int i = 0; i < totalDias; i++) {
				Sessao temp = new Sessao();
				temp.setEspetaculo(this);
				temp.setInicio(inicio.plusDays(i).toDateTime(horario));

				sessoes.add(temp);
			}
		} else {
			for (int i = 0; i < totalDias; i += 7) {
				Sessao temp = new Sessao();
				temp.setEspetaculo(this);
				temp.setInicio(inicio.plusDays(i).toDateTime(horario));
